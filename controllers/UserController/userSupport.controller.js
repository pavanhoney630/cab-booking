...
const raiseTicket = async (req, res) => {
  try {
    ...
    const newTicket = new Ticket({
      ticketId,
      issue,
      userId: req.user._id,           // ✅ add user ID from token
      userType: req.user.type || 'User' // optional: 'user' or 'driver'
    });
...
const sendMessage = async (req, res) => {
  try {
   ...
   const userId = req.user._id // replace this with req.user.id
    ...
    const ticketExists = await Ticket.findOne({ ticketId });
...
const updateTicketStatus = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Failed to Update TicketStatus' });
  }
};
